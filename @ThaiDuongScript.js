...
var uass = `Mozilla/5.0 (${randomOS}; ${randomArch}; rv:${a}.0) Gecko/20100101 Firefox/${a}`

var uasss = `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${a}.0.0.0 Safari/537.36 Brave/${a}.0.0.0`
//console.log(uas)
...
var encoding = encoding_header[Math.floor(Math.floor(Math.random() * encoding_header.length))];
   hd = {}
     header = {
    ':method': Math.random() < 0.5 ? 'GET' : 'GET'
		, ':authority': parsed.host
		, 'x-forwarded-proto':'https'
  };
  if (query === 'true'){
  header[':path'] = Math.random() < 0.5 ? parsed.path + '?' +randstr(5) + '=' + randstr(15) : parsed.path + '?' + 'https://www.gooogle.com/page=' + randstr(5) + '=' + randstr(10) + '?thaiduong=' + randstr(3) + 'GoogleBot' + randstr(2) ;
...
const brw = ['chrome','firefox','edge','macos','linux','yandex','brave']
let dynHeaders
let ci
let bruh 
async function rand() {
