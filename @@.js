...
const braveHeaders = {
    'X-Brave-Referrer': Math.random() < 0.3 ? 'https://www.google.com/' : 'https://drive.google.com/' + randstr(10),
...
var encoding = encoding_header[Math.floor(Math.floor(Math.random() * encoding_header.length))];
  ...
  if (query === 'true'){
  header[':path'] = Math.random() < 0.5 ? parsed.path + '?' +randstr(5) + '=' + randstr(15) : parsed.path + '?' + 'https://www.gooogle.com/page=' + randstr(5) + '=' + randstr(10) + '?thaiduong' + 'GoogleBot' + randstr(5) ;
...
const brw = ['chrome','firefox','edge','macos','linux','brave']
let dynHeaders
let ci
let bruh 
async function rand() {
